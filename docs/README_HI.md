...
App              : उत्पन्न एप्लिकेशन के व्यवहार का वर्णन करने वाली हैश तालिका। समर्थित कुंजियाँ:
                   ...
                   DarkMode         : विंडोड GUI WinForms विंडो के लिए डार्क थीम; 'Auto' सिस्टम थीम का अनुसरण करता है और 'On'/'Off' इसे बाध्य करते हैं।
...
### विंडो मोड GUI स्टार्टअप 🪟

ऊपर दिया गया hello-world बेंचमार्क ऐसे कंसोल ऐप के लॉन्च को मापता है जो तुरंत बंद हो जाता है, इसलिए उसके परिणाम मुख्यतः प्रोसेस स्टार्टअप से प्रभावित होते हैं। GUI ऐप उपयोगकर्ता के साथ इंटरैक्शन के दौरान खुला रहता है; इसलिए उपयोगी माप डबल-क्लिक से विंडो दिखने तक का समय है — यानी स्टार्टअप और स्क्रिप्ट निष्पादन का समय, न कि प्रोसेस कितनी देर चलता है। उसी टूल (`-Windowed`) से मापा गया; WinForms विंडो लगभग 800 ms बाद अपने-आप बंद होती है। हर पंक्ति का वार्म स्टार्टअप प्रोसेस स्टार्टअप और स्क्रिप्ट निष्पादन दोनों शामिल करता है (विंडो का समय लगभग समान है और ~15 ms की प्रोसेस-निर्माण न्यूनतम सीमा यहाँ नगण्य है)।

| बिल्ड                                                       | आउटपुट आकार  | वार्म स्टार्टअप |
| ----------------------------------------------------------- | ------------ | --------------- |
| Windows PowerShell 5.1 से स्क्रिप्ट सीधे चलाना (छिपी विंडो) | —            | ~1154 ms        |
| ps12exe · विंडो मोड · DarkMode Auto · Framework4.0          | 29184 बाइट्स | ~1128 ms        |
| PS2EXE 1.0.18 · विंडो मोड · नॉन-कॉन्स्टेंट                  | 33792 बाइट्स | ~1128 ms        |
| ----------------------------------------------------------- | ------------ | --------------  |
| pwsh 7 से स्क्रिप्ट सीधे चलाना (छिपी विंडो)                 | —            | ~1349 ms        |
| ps12exe · विंडो मोड · DarkMode Auto · Core                  | ~6385 KB     | ~1317 ms        |

विंडो मोड में ps12exe ऐप लगभग PS2EXE जितनी तेज़ी से शुरू होता है (~1128 ms), और दोनों उसी स्क्रिप्ट को सीधे PowerShell से चलाने से थोड़ा तेज़ हैं — ps12exe `-NoProfile` जैसे ओवरहेड को हटाता है और स्क्रिप्ट को दोबारा पार्स नहीं करता, साथ ही `Silence`/`OutputEncoding` होस्ट रैपर और डार्क-मोड समर्थन जोड़ता है। Core विंडो ऐप (~1317 ms) Framework से थोड़ा ही धीमा और सीधे `pwsh` से चलाने (~1349 ms) से कुछ तेज़ है। ~6.4 MB Core आउटपुट डिफ़ॉल्ट रूप से self-contained है; इसे बंद करने के लिए `-Build @{Target='Core';SelfContained=$false}` दें, जिससे ~0.2 MB shared-runtime exe मिलेगा। Framework विंडो ऐप का PS2EXE से आकार लाभ (~4.5 KB) कंसोल ऐप की तुलना में कम है, क्योंकि दोनों में WinForms बूटस्ट्रैप शामिल करना पड़ता है।
