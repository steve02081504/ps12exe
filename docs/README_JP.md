...
ネイティブエクスポートには .NET Framework 4.0 ターゲットと `x86`/`x64` プラットフォームが必要です（`AnyCPU` はホストのビット数に自動解決されます）。出力はデフォルトで `.dll` です。ゲスト（サンドボックス）モードでは利用できません。ネイティブエクスポートは同梱の AsmResolver によりプロセス内で生成され、外部ツールチェーンは不要です。
