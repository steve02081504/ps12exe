...
Build            : ビルドオプションのハッシュテーブル。サポートされるキー：
...
ネイティブエクスポートには .NET Framework 4.0 ターゲットと `x86`/`x64` プラットフォームが必要です（`AnyCPU` はホストのビット数に自動解決されます）。出力はデフォルトで `.dll` です。ゲスト（サンドボックス）モードでは利用できません。呼び出しは直列化され、最初の呼び出しで PowerShell 実行空間が起動します。スクリプトでエラーが起きると stderr に出力され、例外をネイティブ境界の外へ通さずに、宣言した型の既定値を返します。
...
定数のみで副作用のないスクリプトについて、ps12exe はコンパイル時に評価し、その結果を非常に小さな exe（通常 1KB 前後）に直接組み込みます。評価がタイムアウトした場合（デフォルト 7 秒）または結果が長すぎる場合は通常のコンパイルにフォールバックします。評価環境が実行時と異なる場合、あるいは完全な PowerShell ホストが必要な場合は、スクリプトに以下のいずれかの pragma を追加してこの最適化を明示的に無効にできます：
...
| ビルド                                             | 出力サイズ   | ウォーム起動 |
| -------------------------------------------------- | ------------ | ------------ |
| Windows PowerShell 5.1 でスクリプトを直接実行      | —            | ~406 ms      |
| ps12exe · 定数 · Framework4.0                      | 1024 バイト  | ~54 ms       |
| ps12exe · 非定数 · Framework4.0                    | 14848 バイト | ~365 ms      |
| PS2EXE 1.0.18 · 非定数                             | 25088 バイト | ~398 ms      |
| ps12exe · 非定数 · 大規模スクリプト · Framework4.0 | 30208 バイト | ~381 ms      |
| PS2EXE 1.0.18 · 非定数 · 大規模スクリプト          | ~496 KB      | ~402 ms      |
| -------------------------------------------------- | ------------ | ------------ |
| pwsh 7 でスクリプトを直接実行                      | —            | ~676 ms      |
| ps12exe · 定数 · Core                              | ~165 KB      | ~104 ms      |
| ps12exe · 非定数 · Core                            | ~181 KB      | ~621 ms      |
| ps12exe · 非定数 · 大規模スクリプト · Core         | ~187 KB      | ~637 ms      |
| PS2EXE 1.0.18 · 非定数 · Core                      | 非対応       | 非対応       |

定数スクリプトはコンパイル時に評価されるため、exe は 1 KB で PowerShell を起動しません。PS2EXE の hello world より約 24 倍小さく、起動は約 6 倍高速です。非定数 exe も PS2EXE より約 40% 小さく、トップレベル変数を多用するスクリプトでは、スクリプトがグローバルスコープではなく関数内（ローカルスコープ）で実行されるため、実行も速くなります。非定数 exe は常に圧縮され、その効果はスクリプトが大きいほど顕著です。約 0.5 MB のスクリプトでも Framework の出力は約 30 KB にとどまり、PS2EXE の約 496 KB の約 1/16 です（PS2EXE はペイロードをほとんど圧縮せず、スクリプトとともに肥大化します）。Core の出力も小規模スクリプトより約 6 KB 増えるだけで、大きなペイロードでも小さく保たれます。

### ウィンドウ GUI の起動 🪟

上の hello world ベンチマークは、起動後すぐに終了するコンソールアプリを測定しているため、数値の大部分はプロセスの起動時間です。GUI アプリはユーザーが操作している間動作するため、重要なのはプロセスの実行時間ではなく、ダブルクリックからウィンドウ表示までの時間、つまり起動とスクリプト実行のオーバーヘッドです。同じツール（`-Windowed`）を使い、約 800 ms 後に自動で閉じる WinForms ウィンドウで測定しました。各行のウォーム起動時間にはプロセス起動とスクリプト実行が含まれます（ウィンドウの表示時間はほぼ同じで、約 15 ms のプロセス生成下限はここでは無視できます）。

| ビルド                                                            | 出力サイズ   | ウォーム起動 |
| ----------------------------------------------------------------- | ------------ | ------------ |
| Windows PowerShell 5.1 でスクリプトを直接実行（非表示ウィンドウ） | —            | ~1154 ms     |
| ps12exe · ウィンドウ · DarkMode Auto · Framework4.0               | 29184 バイト | ~1128 ms     |
| PS2EXE 1.0.18 · ウィンドウ · 非定数                               | 33792 バイト | ~1128 ms     |
| --------------------------------------------------------------    | ------------ | ------------ |
| pwsh 7 でスクリプトを直接実行（非表示ウィンドウ）                 | —            | ~1349 ms     |
| ps12exe · ウィンドウ · DarkMode Auto · Core                       | ~6385 KB     | ~1317 ms     |

ウィンドウアプリの ps12exe は PS2EXE とほぼ同じ速さで起動し（約 1128 ms）、どちらも同じスクリプトを PowerShell から直接実行するよりわずかに速くなります。ps12exe は `-NoProfile` 相当のオーバーヘッドを省き、スクリプトを再解析せず、さらに `Silence`/`OutputEncoding` ホストラッパーとダークモード対応を加えます。Core のウィンドウアプリ（約 1317 ms）は Framework よりわずかに遅いだけで、スクリプトを `pwsh` から直接実行する場合（約 1349 ms）より少し速くなります。約 6.4 MB の Core 出力は既定で self-contained です。`-Build @{Target='Core';SelfContained=$false}` で無効にすると、約 0.2 MB の共有ランタイム exe にできます。Framework のウィンドウアプリが PS2EXE より小さくなる量（約 4.5 KB）は、どちらも WinForms の起動コードを含むため、コンソールアプリの場合より少なくなります。

### コンパイル速度 ⏱️

同じツール（`-Compile -IncludeCore`）で計測。各サンプルは新しいホストプロセスで実行（Framework/PS2EXE は Windows PowerShell 5.1、Core は pwsh 7）。「ウォーム」は初回以降 5 回のコンパイルの中央値です。PS2EXE の数値はローカルにインストールされたリリース（この環境では 1.0.18）を使用しています。

| ビルド                                             | ウォームコンパイル |
| -------------------------------------------------- | ------------------ |
| ps12exe · 定数 · Framework4.0                      | ~2.3 s             |
| ps12exe · 非定数 · Framework4.0                    | ~1.3 s             |
| PS2EXE · 非定数                                    | ~0.9 s             |
| ps12exe · 非定数 · 大規模スクリプト · Framework4.0 | ~1.5 s             |
| PS2EXE · 非定数 · 大規模スクリプト                 | ~0.7 s             |
| -------------------------------------------------- | ------------------ |
| ps12exe · 定数 · Core                              | ~4.2 s             |
| ps12exe · 非定数 · Core                            | ~5.7 s             |
| ps12exe · 非定数 · 大規模スクリプト · Core         | ~5.9 s             |
| PS2EXE · 非定数 · Core                             | 非対応             |
...
`#_DllExport` を含むスクリプトは、`LoadLibrary`/`GetProcAddress` で呼び出せる Win32 DLL にコンパイルされます（Framework4.0 + x86/x64 のみ。PS2EXE に相当機能はありません）。固定の 2 エクスポートスクリプト（`Add`、`Greet`）：

| ビルド                                    | 出力サイズ   | ウォームコンパイル |
| ----------------------------------------- | ------------ | ------------------ |
| ps12exe · DLL エクスポート · Framework4.0 | 28160 バイト | ~2.8 s             |
| PS2EXE 1.0.18 · DLL エクスポート          | 非対応       | 非対応             |
