...
[input |] ps12exe [[-inputFile] '<nombre de archivo|url>' | -Content '<script>'] [-outputFile '<nombre de archivo>']
        ...
        [-PreprocessOnly] [-Golf] [-Sandbox] [-NoUpdateCheck] [-Quiet] [-Locale '<código de idioma>'] [-ConfigFile] [-help]
```
...
App              : Una tabla hash que describe el comportamiento de la aplicación producida. Claves admitidas:
                   ...
                   DarkMode         : Tema oscuro para las ventanas WinForms de las aplicaciones GUI; 'Auto' sigue el tema del sistema y 'On'/'Off' lo fuerzan.
                   ...
                   ConHost          : Forzar una consola conhost en lugar de Windows Terminal; desactiva la redirección de entrada/salida/error.
...
Build            : Una tabla hash de opciones de compilación. Claves admitidas:
                   ...
                   Core             : Opciones para las compilaciones del destino 'Core'. Claves admitidas:
                                      Backend                : 'Shared' (por defecto) resuelve PowerShell desde la instalación de pwsh de la máquina de destino y mantiene la salida pequeña; 'Bundled' incluye el SDK de PowerShell (Microsoft.PowerShell.SDK), por lo que la máquina de destino no necesita pwsh y SelfContained/Trimmed/ReadyToRun/InvariantGlobalization/Aot pasan a estar disponibles, a costa de una salida mucho mayor.
                                      TargetOs               : Sistema operativo de destino: 'Windows', 'Linux' o 'MacOS' (por defecto: el SO de la máquina de compilación). La salida GUI/con ventana requiere 'Windows'.
                                      TargetFramework        : Moniker de framework .NET de destino (por ejemplo 'net8.0'). Por defecto, el tiempo de ejecución de la máquina de compilación (Shared) o el framework asignado a PowerShellVersion (Bundled).
                                      PowerShellVersion      : Versión del SDK de PowerShell incluido (sólo Bundled). Por defecto, la versión de PowerShell de la máquina de compilación.
                                      SingleFile             : Publicar un ejecutable de archivo único (por defecto $true). Si es $false, el ejecutable y sus dependencias se escriben como una carpeta.
                                      SelfContained          : Incluir el tiempo de ejecución de .NET (sólo Bundled; por defecto $false). Mucho más grande pero no necesita un tiempo de ejecución instalado.
                                      Trimmed                : Habilitar el recorte de IL para reducir el tamaño (sólo Bundled; por defecto $false).
                                      TrimMode               : Agresividad del recorte cuando Trimmed está establecido: 'partial' (por defecto, seguro) o 'full' (agresivo, puede romper la reflexión).
...
Quiet            : Suprimir la salida informativa (del host) durante la compilación; los errores y advertencias se siguen mostrando.
Locale           : El código de idioma que desea usar.
...
El benchmark de hello world anterior mide el lanzamiento de una aplicación de consola que termina inmediatamente, por lo que sus cifras están dominadas por el arranque del proceso. Una aplicación GUI permanece abierta mientras el usuario interactúa; por eso, la medida relevante es el tiempo desde el doble clic hasta que aparece la ventana —el coste de inicio y ejecución del script, no la duración total del proceso—. Se midió con la misma herramienta (`-Windowed`), usando una ventana WinForms que se cierra sola tras ~800 ms; el arranque en caliente de cada fila incluye el inicio del proceso y la ejecución del script (la duración de la ventana es casi idéntica en todas las filas y el mínimo de ~15 ms de creación del proceso resulta insignificante aquí).

| Compilación                                                               | Tamaño de salida | Inicio en caliente |
| ------------------------------------------------------------------------- | ---------------- | ------------------ |
| Windows PowerShell 5.1 ejecutando el script directamente (ventana oculta) | —                | ~1154 ms           |
| ps12exe · modo ventana · DarkMode Auto · Framework4.0                     | 29184 bytes      | ~1128 ms           |
| PS2EXE 1.0.18 · modo ventana · no constante                               | 33792 bytes      | ~1128 ms           |
| -------------------------------------------------------------------       | ---------------- | ------------------ |
| pwsh 7 ejecutando el script directamente (ventana oculta)                 | —                | ~1349 ms           |
| ps12exe · modo ventana · DarkMode Auto · Core                             | ~6385 KB         | ~1317 ms           |

Una aplicación ps12exe en modo ventana arranca casi tan rápido como PS2EXE (~1128 ms), y ambas son algo más rápidas que ejecutar el mismo script directamente con PowerShell: ps12exe elimina costes similares a `-NoProfile` y no vuelve a analizar el script, aunque añade el envoltorio del host `Silence`/`OutputEncoding` y compatibilidad con el modo oscuro. La aplicación Core en modo ventana (~1317 ms) solo es ligeramente más lenta que Framework y un poco más rápida que `pwsh` ejecutando el script directamente (~1349 ms). La salida Core de ~6,4 MB es self-contained de forma predeterminada; desactívelo con `-Build @{Target='Core';SelfContained=$false}` para usar el ejecutable de ~0,2 MB con runtime compartido. La ventaja de tamaño de la aplicación Framework en modo ventana frente a PS2EXE (~4,5 KB) es menor que en las aplicaciones de consola porque ambas deben incluir el inicio de WinForms.
