...
上面的 hello world 基准测量的是启动后立即退出的控制台程序，因此结果主要受进程启动时间影响。GUI 程序会在用户交互期间持续运行，更有意义的指标是从双击到窗口出现的时间，也就是启动和脚本执行开销，而不是进程存活时间。使用同一工具（`-Windowed`）测量，WinForms 窗口会在约 800 ms 后自行关闭；每行的预热启动时间都包含进程启动和脚本执行（各项窗口存活时间几乎相同，约 15 ms 的进程创建下限在这里可以忽略）。

| 构建                                              | 输出体积   | 预热启动 |
| ------------------------------------------------- | ---------- | -------- |
| 直接运行脚本的 Windows PowerShell 5.1（隐藏窗口） | —          | ~1154 ms |
| ps12exe · 窗口化 · DarkMode Auto · Framework4.0   | 29184 字节 | ~1128 ms |
| PS2EXE 1.0.18 · 窗口化 · 非常量                   | 33792 字节 | ~1128 ms |
| ------------------------------------------------- | ---------- | -------- |
| 直接运行脚本的 pwsh 7（隐藏窗口）                 | —          | ~1349 ms |
| ps12exe · 窗口化 · DarkMode Auto · Core           | ~6385 KB   | ~1317 ms |

窗口化 ps12exe 应用的启动速度与 PS2EXE 大致相同（约 1128 ms），两者都比直接通过 PowerShell 运行同一脚本略快——ps12exe 减少了类似 `-NoProfile` 的开销且不会重新解析脚本，同时仍加入了 `Silence`/`OutputEncoding` 主机包装和暗色模式支持。Core 窗口化应用约需 1317 ms，仅比 Framework 稍慢，也略快于直接运行脚本的 pwsh（约 1349 ms）。默认情况下，约 6.4 MB 的 Core 产物是 self-contained；禁用它可通过 `-Build @{Target='Core';SelfContained=$false}` 回退到约 0.2 MB 的共享运行时 exe。Framework 窗口化应用相比 PS2EXE 的体积优势（约 4.5 KB）小于控制台程序，因为两者都必须嵌入 WinForms 启动代码。
