...
.DESCRIPTION
	用本仓库中的 ps12exe 模块和本地安装的最新 PS2EXE 编译常量 / 非常量（可选 Core）脚本，然后打印与 README“Size & Speed Benchmark”表相呼应的 markdown（相同的行、标签、组分隔符和大小格式）。-ProbeRuntime 还会打印“Compiled-EXE Runtime Behaviour”表。PS2EXE 永不下载：使用本地安装的最新 ps2exe 模块。找不到时会跳过其行，并在末尾打印安装提示。要求：Windows、Windows PowerShell 5.1、pwsh 7+，以及使用 -IncludeCore 时的 .NET SDK。
.PARAMETER Runs
	每个目标运行时测速的热运行次数。默认为 20。
.PARAMETER IncludeCore
	同时构建 Core（PowerShell 7+）目标。需要 .NET SDK，且更慢。
.PARAMETER Compile
	额外测量各编译器的编译耗时（每个样本用一个全新的宿主进程，含进程启动）。打印“Compilation Speed”表。
.PARAMETER CompileRuns
	编译耗时的计时样本数（先做一次预热，再取中位数）。默认为 3。
.PARAMETER Script
...
'@
	...
	function Measure-Compile([string]$Id, [string]$Label, [string]$Compiler, [string]$Target, [string]$Src) {
		...
		$out = Join-Path $tempDir ("c_{0}_{1}.exe" -f $Id, [guid]::NewGuid().ToString('N').Substring(0, 8))
		$invoke = {
