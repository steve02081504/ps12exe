@{
	...
	UnsafeWarning		   = "警告：このサービスは安全ではありません。ps12exeは、作者が考慮していない特定のスクリプトをコンパイルする際に、意図しない副作用のあるコードを実行する可能性があります。"
	...
	GUIHelpData			   = @{
		title	   = "使用方法："
		Usage	   = "ps12exeGUI [[-ConfingFile] '<設定ファイル>'] [-Localize '<言語コード>'] [-UIMode 'Dark'|'Light'|'Auto'] [-help]"
		PrarmsData = [ordered]@{
			ConfingFile = "読み込む設定ファイル。"
			Localize	= "使用する言語コード。"
