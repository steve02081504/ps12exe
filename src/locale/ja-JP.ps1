...
"@
	...
	exe21spHelpData			= @{
		title	   = "用法："
		Usage	   = "[input |] exe21sp [-ExePath] '<exeファイルパス>' [-OutFile '<出力ps1パス>'] [-help]"
		PrarmsData = [ordered]@{
			input    = "反コンパイルする ps12exe 生成 exe のパス（パイプ入力時）。"
			ExePath  = "反コンパイルする ps12exe 生成 exe のパス。"
			OutFile  = "省略可。復元スクリプトを書き出す ps1 のパス。省略時はリダイレクト時は標準出力へ、そうでなければ同ディレクトリの ``<exe>.ps1`` に書き出す。"
			help	 = "このヘルプを表示。"
		}
	}
	CompilingI18nData		= @{
	...
	exe21spInteractI18nData = @{
		...
		EnterExePath		 = "ps12exe で生成した exe のパスを入力してください（終了するには空白）："
		EnterOutputPs1Path	 = "出力 ps1 パスを入力してください（空白の場合は同じフォルダの <exe>.ps1 を使用）："
		Prompt				 = " >> "
		AdditionalInfoPrompt = "[Y/N]"
