...
/** @returns {{ text: string, indentUnit: string } | undefined} */
function loadWorkspaceTarget () {
	const file = path.join(REPO_ROOT, 'ps12exe.ps1')
...
suite('ps12exe formatter', () => {
	test('the emulated official formatter preserves the workspace style', async function () {
		this.timeout(120000)
		const target = loadWorkspaceTarget()
		if (!target) this.skip()
		...
		assert.strictEqual(formatted, target.text, 'formatting ps12exe.ps1 must not change it')
	})

	test('formatting ps12exe.ps1 is idempotent', async function () {
		this.timeout(120000)
		const target = loadWorkspaceTarget()
		if (!target) this.skip()
		const host = await resolvePlainPowerShell()
