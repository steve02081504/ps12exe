...
const PRAGMA_NAME_RE = /^([ \t]*)#_pragma[ \t]+([a-zA-Z_][a-zA-Z_0-9]*(?:\.[a-zA-Z_][a-zA-Z_0-9]*)*)/
 ...
 */
function pragmaNameAt (line, character) {
 ...
 */
async function getPragmaData (locale) {
 ...
 *
 * @param {Map<string, { name: string, description: string }>} data
 * @param {string} name
 * @returns {{ name: string, description: string, negated: boolean } | null}
 */
function lookupPragma (data, name) {
 ...
 * @param {Map<string, { name: string, description: string }>} data
 * @param {string} prefix
 * @returns {Array<{ name: string, insertText: string, kind: 'object' | 'value', description: string }>}
 */
function buildPragmaCandidates (data, prefix) {
...
function clearPragmaCache () {
	cache.clear()
}

export { pragmaNameAt, getPragmaData, lookupPragma, buildPragmaCandidates, clearPragmaCache }
