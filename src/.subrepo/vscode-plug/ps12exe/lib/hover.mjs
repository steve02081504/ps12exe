// ps12exe preprocessor 指令的悬浮提示支持：本地化的说明，以及指向该区域 README 中对应
// 「预处理」小节的链接。
//
// 每个指令映射到 README 的一个小节。所有区域的 README（docs/README_*）都在小节旁埋了统一的
// 显式锚点（`<a id="preprocessing-…">`），因此链接不依赖各语言标题自动生成的锚点，切换语言
// 也不会失效。`#_DllExport` 未在 README 中单独记录，回退到「预处理」概览小节。
...
const HOVER_MESSAGES = Object.freeze({
	...
	pragma: '`#_pragma <name> [value]` sets a compilation parameter such as `Console`, `noConsole`, `icon` or `title` without modifying the script.',
...
// 带边界的关键字指令（`#_if`、`#_include_as_value` …）；`\b` 提供边界，因此 `#_iffy` 或
// `#_include_other` 这类未知指令不会被误认。
...
/**
 * 返回某个区域 README 中对应小节的链接；未知区域回退到 en-UK（与官网语言重定向页一致），
 * 未知小节回退到「预处理」概览小节。
 *
