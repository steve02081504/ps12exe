...
- 所有参数都要转发或转写：
  - 能直接对应 ps12exe 参数的，做映射后转发（`x86/x64 → Build.Platform`、`STA/MTA → Build.Apartment`、资源参数 → `-Resources @{…}`、`-noConsole → App.Windowed`、`-noOutput/-noError → App.Silence`）。
  - ps12exe 没有的能力（`conHost`、`embedFiles`）以及 PS2EXE 运行时变量（`$ScriptRoot`），通过 `-Build @{Minify=…}` 在预处理后改写脚本实现。
