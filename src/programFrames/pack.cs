...
// ps12exe 把真正的托管程序集压缩后作为 "main" 资源塞进这个 launcher，
// launcher 启动时在内存里解压、Assembly.Load 后调用负载的入口点，负载不落盘。
// Windows PowerShell（CodeDom）下负载用 gzip；PowerShell Core 下用 Brotli（压缩率更高，但 .NET Framework 不提供 BrotliStream）。
// 版本/标题等资源参数由 ps12exe 替换下面的 $placeholder（见 CodeDomCompiler.ps1 / CoreCompiler.ps1），
